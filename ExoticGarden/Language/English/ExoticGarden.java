...
import me.mrCookieSlime.ExoticGarden.CustomFood;
import me.mrCookieSlime.ExoticGarden.CustomJuice;
import me.mrCookieSlime.ExoticGarden.EGPlant;
import me.mrCookieSlime.ExoticGarden.FoodListener;
import me.mrCookieSlime.ExoticGarden.PlantData;
import me.mrCookieSlime.ExoticGarden.PlantType;
import me.mrCookieSlime.ExoticGarden.PlantsListener;
...
public class ExoticGarden extends JavaPlugin {
	...
	public static String Read_Config(String get_file, String get_section) {
		 File file = new File("plugins/" + get_file + ".yml");
		 FileConfiguration data = YamlConfiguration.loadConfiguration(file);
		 return data.getString(get_section);
		}
	
	private static boolean skullitems;
	@SuppressWarnings("deprecation")
	public void onEnable(Config cfg) {
        Bukkit.getScheduler().scheduleSyncRepeatingTask(ExoticGarden.getPlugin(), new Runnable() {
        	public void run() {
        	for(World World : Bukkit.getWorlds()) {
        		for (Entity Player : World.getEntities()) {
        			if (Player instanceof Player) { int Slot = 0;
        			for (ItemStack ItemStack : ((HumanEntity) Player).getInventory().getContents()) {
                        if(ItemStack != null && ItemStack.getType() != Material.AIR && ItemStack.hasItemMeta() && ItemStack.getItemMeta().hasLore()) {
                            if (cfg(Language, Language.english).matches("(eng|jp|nl)")) {
                                LanguageHelper.getItemDisplayName(ItemStack, Config(Language, Language.english));
                            }
                        }
                    }
                }
		...
		if (loader.load()) {
			...
			registerTree("Apple Oak", new MaterialData(Material.APPLE), "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvY2JiMzExZjNiYTFjMDdjM2QxMTQ3Y2QyMTBkODFmZTExZmQ4YWU5ZTNkYjIxMmEwZmE3NDg5NDZjMzYzMyJ9fX0=", "사과", "&c", 0, "사과 주스", true, Material.DIRT, Material.GRASS);
			registerTree("Coconut", new MaterialData(Material.INK_SACK, (byte) 3), "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNmQyN2RlZDU3Yjk0Y2Y3MTViMDQ4ZWY1MTdhYjNmODViZWY1YTdiZTY5ZjE0YjE1NzNlMTRlN2U0MmUyZTgifX19", "야자", "&6", 8194, "야자 과즙", false, Material.SAND);
